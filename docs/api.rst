...
The ApiPermission.Class is commented which allows API endpoint access to any user. When uncommented then user rights are checked before responding to any API endpoint.  Call to bus controller needs to have given user credentials. The user is checked in AAF for permission to call topic.
